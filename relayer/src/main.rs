...
fn cli() -> Command<'static> {
    Command::new("relayer")
        ...
        .subcommand(Command::new("increment").about("Increments N1 by one using DAPP account"))
}
...
pub async fn run(cmd: &str, settings: &Settings) {
    ...
    let provider_n1 = Provider::<Http>::try_from(settings.r.n1_rpc_url.clone())
        .expect("could not instantiate RPC Provider N1");

    let provider_n2 = Provider::<Http>::try_from(settings.r.n2_rpc_url.clone())
    ...
    let dapp_init_nonce_n1 = provider_n1
        .get_transaction_count(dapp_address, Some(last_block_n1))
        .await
        .unwrap();
    let amb_init_nonce_n2 = provider_n2
        .get_transaction_count(amb_address, Some(last_block_n2))
        .await
        .unwrap();
    ...
    match cmd {
        "increment" => {
            ...
            let mut increment_n1_call = counter_n1_sc_dapp
                .increment(dapp_address, U256::zero())
                .from(dapp_address)
            ...
            let mut send_from_n1_call = counter_n1_sc_dapp
                .send()
                .from(dapp_address)
                .gas_price(gas_price_n1)
                .gas(gas_limit_n1);
            ...
            match send_from_n1_call.send().await {
                ...
                Err(e) => {
                    println!(
                        "Error during sending value of Counter N1 over to N2: {:?}",
                        e
                    );
                }
            };
        ...
        "relayer" => {
            ...
            loop {
                new_value_to_send = counter_n1_sc_amb
                    .get_target_value(counter_n2_addr)
                    .call()
                    .await
                    .unwrap();
                ...
                if new_value_to_send > orig_value_to_send {
                    println!(
                        "Going to update Counter N2 to value {:?} from N1...\n",
                        new_value_to_send
                    );
                    let mut increment_n2_call = counter_n2_sc_amb
                        .increment(counter_n2_addr, new_value_to_send)
                        .from(amb_address)
